...
### Programmatic Usage Summary (API)

You can query your current usage summary using your API key. This endpoint returns only the essential fields required for monitoring and alerting.
...
```bash
curl -X GET -H "X-API-Key: YOUR_API_KEY" -H "Content-Type: application/json" https://sim.ai/api/users/me/usage-summary
```
